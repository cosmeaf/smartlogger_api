    ...
    try:
        while True:
            ...
            if ']' in linha:
                ...
                # Verifica se a parte restante começa com 'STT', 'ALT', ou 'UEX'
                if parte.startswith('STT') or parte.startswith('ALT') or parte.startswith('UEX'):
                    ascii_string = parte  # Define a variável ascii_string
                    
                    # Processamento dos dados dependendo do prefixo
                    if ascii_string.startswith('UEX') or ascii_string.startswith('STT') or ascii_string.startswith('ALT'):
                        ...
                        # Diferentes campos para diferentes tipos de mensagens
                        if ascii_string.startswith('UEX'):
                            dados_completos['data_length'] = safe_int(dados[16]) if len(dados) > 16 and dados[16] else None
                            dados_completos['driver_id'] = dados[17] if len(dados) > 17 else None
                            dados_completos['instant_speed'] = safe_float(dados[18]) if len(dados) > 18 and dados[18] else None
                            dados_completos['peak_speed'] = safe_float(dados[19]) if len(dados) > 19 and dados[19] else None
                            dados_completos['instant_temperature'] = safe_float(dados[20]) if len(dados) > 20 and dados[20] else None
                            dados_completos['peak_temperature'] = safe_float(dados[21]) if len(dados) > 21 and dados[21] else None
                        elif ascii_string.startswith('STT'):
                        ...
                        elif ascii_string.startswith('ALT'):
                            dados_completos['alert_id'] = dados[16] if len(dados) > 16 else None
                            dados_completos['alert_modifier'] = dados[17] if len(dados) > 17 else None
                            dados_completos['alert_data'] = dados[18] if len(dados) > 18 else None
                            dados_completos['reserved'] = dados[19] if len(dados) > 19 else None
                            dados_completos['assign_map'] = dados[20] if len(dados) > 20 else None
                            dados_completos['power_voltage'] = safe_float(dados[21]) if len(dados) > 21 and dados[21] else None
                            dados_completos['battery_voltage'] = safe_float(dados[22]) if len(dados) > 22 and dados[22] else None
