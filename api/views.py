...
class DeviceViewSet(mixins.ListModelMixin, 
    ...
    """
    queryset = Device.objects.all()  # Para retornar todos os dispositivos
    serializer_class = DeviceSerializer

    def get_queryset(self):
        available = self.request.query_params.get('available', None)
        if available == 'true':
            # Retorna dispositivos que não estão associados a nenhum Equipment
            return Device.objects.filter(equipments__isnull=True)
        # Se `available` não for passado, retorna todos os dispositivos
